...
def laplacian():
    ...
    return eigenvalues_smpl, eigenvectors_smpl, eigenvalues_rabit, eigenvectors_rabit, laplacian_dense, laplacian_dense_r
...
def compute_hks(evals, evecs, count):
    ...
    scales = torch.logspace(-2, 0., steps=count, dtype=torch.float32)
    ...
    power_coefs = torch.exp(-evals.unsqueeze(1) * scales.unsqueeze(-1)).unsqueeze(1)
    terms = power_coefs * (evecs * evecs).unsqueeze(2)
...
def compute_mat(k, f_dim, eval_smpl, evec_smpl, eval_rabit, evec_rabit, joints, joints_rabit):
    hks_smpl = compute_hks(eval_smpl[:k], evec_smpl[:,:k], f_dim) #(B,V,feature_dim)
    hks_rabit = compute_hks(eval_rabit[:k], evec_rabit[:,:k], f_dim) #(B,V,feature_dim)
        ...
        for j in range(num_joints):
            for k in range(num_joints):
                for l in range(num_joints):
                    ...
                    corresp_mat[(i*num_joints)+j, (k*num_joints)+l] = ((node1-node2)**2).mean()
    ...
    print(f"\n Correspondence Matrix: {norm_corresp_mat} and Shape: {norm_corresp_mat.shape}")
    
    # plt.imshow(norm_corresp_mat)
    ...
    D = np.zeros_like(norm_corresp_mat)

    for i in range(norm_corresp_mat.shape[0]):
        row_sum = np.sum(norm_corresp_mat[i, :])
        D[i, i] = row_sum
    breakpoint()
    print(f"Shape of the Diagonal Matrix: {D.shape}, \n and D: {D} ")

    L = norm_corresp_mat - D
    breakpoint()
...
if __name__ == '__main__':
    ...
    eigenvalues_smpl, eigenvectors_smpl, eigenvalues_rabit, eigenvectors_rabit, laplacian_dense, laplacian_dense_r = laplacian()
    ...
    corresp_mat = compute_mat(K, feature_dim, eigenvalues_smpl, eigenvectors_smpl, eigenvalues_rabit, eigenvectors_rabit, joints, rabit_joints)

...
def laplacian_mesh():
    ...
    return eigenvalues_smpl_mesh, eigenvectors_smpl_mesh, eigenvalues_rabit_mesh, eigenvectors_rabit_mesh


def get_mesh_data(filename1, filename2, k, f_dim, eval_smpl, evec_smpl, eval_rabit, evec_rabit):
    smpl = trimesh.load_mesh(filename1)
    rabit = trimesh.load_mesh(filename2)
    joints = load_smpl(SMPL_PATH)
    rabit_joints = load_rabit(RABIT_PATH)
    ...
    hks_smpl_mesh = compute_hks(eval_smpl[:k], evec_smpl[:,:k], f_dim)
    hks_rabit_mesh = compute_hks(eval_rabit[:k], evec_rabit[:,:k], f_dim)
                            
    ps.register_surface_mesh("SMPL", smpl.vertices, smpl.faces, transparency=0.3) # mesh of SMPL
    ps.register_curve_network("SMPL Skeleton", joints, EDGES_SMPL, color=(0,1,0)) # SMPL skeleton

    ps.register_surface_mesh("RaBit", (rabit.vertices+[1.0, -0.3, 0.0])* 1.5, rabit.faces, transparency=0.3) # rabit Mesh
    ps.register_curve_network("RaBit Skeleton", (rabit_joints+[1.0, -0.3, 0.0])* 1.5, EDGES_RABIT, color=(1,0,0)) # rabit skeleton
...
def compute_mat(k, f_dim, eval_smpl, evec_smpl, eval_rabit, evec_rabit, joints, joints_rabit, adj_sparse):
    ...
    node_mat = torch.zeros((num_joints, num_joints))
    ...
    for i in range(num_joints):
        for j in range(num_joints):
            node1 = ((hks_smpl[0][i] - hks_rabit[0][j])**2)
            node_mat[i,j] = np.exp(-(node1/(2*sigma**2))).mean()
        ...
        for j in range(num_joints):
            for k in range(num_joints):
                for l in range(num_joints):
                    ...
                    corresp_mat[(i*num_joints)+j, (k*num_joints)+l] = np.exp(-((node1-node2)**2)/(2*sigma**2))
                    # corresp_mat[(i*num_joints)+j, (k*num_joints)+l] = np.mean((smpl_mat[i][j] - rabit_mat[k][l])**2)
...
#     norm_corresp_mat[i] = (corresp_mat[i] - corresp_mat[i].min()) / (corresp_mat[i].max() - corresp_mat[i].min())
    ...
    #     for j in range(node_mat.shape[1]):
    #         plt.text(j, i, f'{node_mat[i, j]:.1f}', ha='center', va='center', color='white')
    ...
    # plt.show()
    sns.heatmap(node_mat, annot=True)
    plt.show()
    ...
    A = nx.from_numpy_array(node_mat)
    ...
    np.fill_diagonal(node_mat, 0) # diagonal entries = 0
    GG = nx.from_numpy_array(node_mat)
    nx.draw(GG, with_labels=True, node_color='lightblue', edge_color=edge_colors)
    plt.show()

    '''Storing the correspondences in a tuple:
    m = 24,24
    m.shape = [rows, cols]
    no of rows = m.shape[0]
    np of cols = m.shape[1]
    '''
    possible_corresp = []
    
    for i in range(node_mat.shape[0]):
        for j in range(node_mat.shape[1]):
            print(f"{i} of Rabit is matching with {j} of SMPL: {node_mat[i,j]}")
            possible_corresp.append([i,j])

    print(f"\n All the correspondences: {possible_corresp}")
    ...
    #     for j in range(node_mat.shape[1]):
    #         rev = [j,i]
    #         if (rev in possible_corresp) and (rev not in final_corresp) and (node_mat[i,j] >= 0.5):
    #             final_corresp.append([i,j])

    breakpoint()
    print(f"\n All the correspondences: {final_corresp}")

    # for pair in possible_corresp:
    #     reverse_pair = [pair[1], pair[0]]
        
    #     if reverse_pair in possible_corresp and reverse_pair != pair and reverse_pair not in matching_pairs:
    #         matching_pairs.append(pair)

    
    breakpoint()
    # checking the Bijectivity of the correspondenes
    # possible_corresp = np.array(final_corresp)
    # smpl_counts = np.bincount(possible_corresp[:,0])
    # rabit_counts = np.bincount(possible_corresp[:,1])

    # bijective_corresp = []
    
    # for smpl_idx, rabit_idx in possible_corresp:
    #     if smpl_counts[smpl_idx] == 1 and rabit_counts[rabit_idx] == 1:
    #         print(f"\n {smpl_idx} of SMPL is matching with {rabit_idx} of RaBit")
    #         bijective_corresp.append([smpl_idx, rabit_idx])
            

    
    plot_corresp(final_corresp)
...
def plot_corresp(corresp):
    ...
    # plotting for smpl
    plt.scatter(joints[:, 0], joints[:, 1], c='r', s=100, label='SMPL Skeleton')
    for edge in edges_smpl:
    ...
    offset = np.array([2.0, 0.5, 0.0])
    rabit_joints = rabit_joints*3.0 + offset

    # plotting for rabit
    plt.scatter(rabit_joints[:, 0], rabit_joints[:, 1], c='b', s=100, label='RaBit Skeleton')
    for edge_r in edges_rabit:
    ...
    for (smpl_idx, rabbit_idx) in corresp:
        plt.plot([joints[smpl_idx, 0], rabit_joints[rabbit_idx, 0]],
                [joints[smpl_idx, 1], rabit_joints[rabbit_idx, 1]],
...
if __name__ == '__main__':
    ...
    eigenvalues_smpl_mesh, eigenvectors_smpl_mesh, eigenvalues_rabit_mesh, eigenvectors_rabit_mesh = laplacian_mesh()

    get_mesh_data(SMPL_MESH, RABIT_MESH, K, feature_dim, eigenvalues_smpl_mesh, eigenvectors_smpl_mesh, eigenvalues_rabit_mesh, eigenvectors_rabit_mesh)
    breakpoint()
